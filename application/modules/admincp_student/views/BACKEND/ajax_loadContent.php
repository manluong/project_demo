							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									<tr role="row">
										...
										<th class="center" width="150">Nam</th>
										<th class="center" width="150">Email</th>
										<th class="center" width="150">Phone</th>
										<th class="center" width="150">Batch No.</th>
										<th class="center sorting" width="80" onclick="sort('created')" id="created">Last Sign in </th>
										...
										if($result){
											...
											foreach($result as $k=>$v)
											{
												...
												<tr class="item_row<?=$i?> gradeX <?php echo $gradeX; ?>" role="row">
													...
													<td class="center"><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->firstname.' '.$v->lastname?></a></td>
													<td class="center"><?=$v->email?></td>
													<td class="center"><?=$v->phone?></td>
													<td class="center"><?=$v->batch_no?></td>
													<td class="center"><?=date('d-m-Y H:i:s',strtotime($v->last_sign_in))?></td>
