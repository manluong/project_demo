...
class Admincp_student extends MX_Controller {

	private $module = 'admincp_student';
	...
	public function admincp_add_student_loan(){
		if($this->model->saveStudentLoan()){
			print 'success';
		}else{
			print 'fail';
		}
	...
	public function admincp_update_student_loan(){
		if($this->model->updateStudentLoan()){
			print 'success';
		}else{
			print 'fail';
		}
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_AVATAR;
	...
	public function admincp_delete(){
		...
		if($this->input->post('id')){
			...
			if($this->db->delete($this->table)){
				//Xóa hình khi Delete
				@unlink(BASEFOLDER.DIR_UPLOAD_AVATAR.$result[0]->image);
				print 'success.'.$this->security->get_csrf_hash();
				exit;
			}
		}
