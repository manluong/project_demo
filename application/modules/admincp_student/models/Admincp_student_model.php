<?php
class Admincp_student_model extends CI_Model {
	...
	function updateStudentLoan(){
		...
		$data = array(
			$field		=> $this->input->post('content') 
		);
		$this->db->where('id', $this->input->post('id'));
		
		if($this->db->update('student_loan',$data)){
			return true;
		}
	}
	...
	function getStudentLoan($id){
		$this->db->select('*');
		$this->db->where('student_id',$id);
		$this->db->from('student_loan');
